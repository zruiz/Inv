...
function ihc_init_form_action($url){
	...
	switch ($_REQUEST['ihcaction']){
		...
		case 'renew_cancel_delete_level_ap'://delete_cancel_level_ap
			global $current_user;
			...
			if (isset($_POST['ihc_renew_level']) && $_POST['ihc_renew_level']){
				ihc_renew_level($current_user->ID, $_POST['ihc_renew_level']);
			}
...
function ihc_renew_level($u_id, $l_id){
	/*
	 *
	* @param user id, level id
	* @return none
