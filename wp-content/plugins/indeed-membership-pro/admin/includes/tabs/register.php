...
switch ($subtab){
			...
			<form action="" method="post">
				<div class="ihc-stuffbox">
					...
					<div class="inside">
						...
						<div style="margin-top: 15px;">
							<input type="submit" value="<?php _e('Save', 'ihc');?>" name="ihc_save_custom_field" onClick="" class="button button-primary button-large" />
						</div>	
					</div>
				</div>
			</form>		
	...
	case 'add_edit_cf':
		...
		$disabled = '';
		if ($meta_arr['name']=='confirm_email' || $meta_arr['name']=='tos' || $meta_arr['name']=='recaptcha' || $meta_arr['name']=='ihc_avatar' || $meta_arr['name']=='ihc_coupon') $disabled = 'disabled';
			...
			<form method="post" action="<?php echo $url.'&tab=register&subtab=custom_fields';?>">
				<div class="ihc-stuffbox">
					...
					<div class="inside">
									...
									if (isset($meta_arr['values']) && $meta_arr['values']){
										foreach ($meta_arr['values'] as $value){
											...
											<div style="display: block;">
												<input type="text" name="values[]" value="<?php echo $value;?>"/>
						...
						<div class="iump-form-line iump-no-border">
						<h2>Labels</h2>
							<label class="iump-labels"><?php _e('Field Label:', 'ihc');?> </label> <input type="text" name="label" value="<?php echo $meta_arr['label'];?>"/>
						</div>							
						<div class="iump-form-line">
							<label class="iump-labels"><?php _e('SubLabel:', 'ihc');?></label>
							<input type="text" value="<?php echo @$meta_arr['sublabel']?>" name="sublabel" style="width: 400px;" />
						</div>		
						<?php if (empty($meta_arr['class'])) $meta_arr['class'] = '';?>
						<div class="iump-form-line iump-no-border">
							<label class="iump-labels"><?php _e('style Class:', 'ihc');?> </label> <input type="text" name="class" value="<?php echo $meta_arr['class'];?>"/>
						</div>	
						<div style="margin-top: 15px;">
