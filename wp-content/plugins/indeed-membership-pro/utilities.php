...
function ihc_print_form_login($meta_arr){
	...
	$sm_string = (!empty($meta_arr['ihc_login_show_sm'])) ? ihc_print_social_media_icons() : '';
...
function ihc_print_social_media_icons($type='login', $already_registered_sm=array()){
	/*
	 * @param string (login, register, update)
	 * @return string
	 */
...
function indeed_create_form_element($attr=array()){
	...
	if (isset($attr['type']) && $attr['type']){
		switch ($attr['type']){
			case 'text':
				$str = '<input type="text" name="'.$attr['name'].'" id="'.$attr['id'].'" class="'.$attr['class'].'" value="'.$attr['value'].'" placeholder="'.$attr['placeholder'].'" '.$attr['other_args'].' '.$attr['disabled'].' />';
				if (!empty($attr['sublabel'])){
					$str .= '<label class="iump-form-sublabel">' . $attr['sublabel'] . '</label>';
				}
				break;
			...
			case 'textarea':
				$str = '<textarea name="'.$attr['name'].'" id="'.$attr['id'].'" class="iump-form-textarea '.$attr['class'].'" '.$attr['other_args'].' '.$attr['disabled'].' >'.$attr['value'].'</textarea>';
				if (!empty($attr['sublabel'])){
					$str .= '<label class="iump-form-sublabel">' . $attr['sublabel'] . '</label>';
				}				
				break;
			...
			case 'checkbox':
				...
				if ($attr['multiple_values']){
					...
					foreach ($attr['multiple_values'] as $v){
						...
						$str .= '<input type="checkbox" name="'.$attr['name'].'[]" id="'.$attr['id'].'" class="'.$attr['class'].'" value="'.$v.'" '.$checked.' '.$attr['other_args'].' '.$attr['disabled'].'  />';
						$str .= $v;
						$str .= '</div>';
					}
					$str .= '</div>';
			...
			case 'radio':
				...
				if ($attr['multiple_values']){
					...
					foreach ($attr['multiple_values'] as $v){
						...
						$str .= '<input type="radio" name="'.$attr['name'].'" id="'.$attr['id'].'" class="'.$attr['class'].'" value="'.$v.'" '.$checked.' '.$attr['other_args'].' '.$attr['disabled'].'  />';
						$str .= $v;
						$str .= '</div>';
					}
					$str .= '</div>';
			...
			case 'submit':
				$str = '<input type="submit" value="'.$attr['value'].'" name="'.$attr['name'].'" id="'.$attr['id'].'" class="'.$attr['class'].'" '.$attr['other_args'].' '.$attr['disabled'].' />';
				if (!empty($attr['sublabel'])){
					$str .= '<label class="iump-form-sublabel">' . $attr['sublabel'] . '</label>';
				}				
				break;
...
function ihc_get_all_post_types(){
	 ...
	 * @param none
	 * @return all the custom post type available in db
	 */
	global $wpdb;
...
function ihc_get_wp_roles_list(){
    ...
    if (!empty($roles)){
    	unset($roles['administrator']);// delete admin role from our list
    	return $roles;
    }
	return FALSE;
}
...
function ihc_print_bank_transfer_order($u_id, $l_id){
	...
	return '<div class="ihc-bank-transfer-msg" id="ihc_bt_success_msg">' . $msg . '</div>';
}
...
function ihc_dont_pay_after_discount($level_id, $coupon, $level_arr, $update_coupon_count=FALSE){
	 ...
	 * @param level_id - int, coupon - string, level_arr - array
	 * @return boolean ,
	 */
	if (!empty($coupon)){
