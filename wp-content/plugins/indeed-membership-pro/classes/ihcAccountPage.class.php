	...
	class ihcAccountPage{
		...
		private function overview_page(){
			...
			if ($post_overview && $post_overview!=-1){
				...
				$this->settings['ihc_ap_overview_msg'] = htmlspecialchars_decode($this->settings['ihc_ap_overview_msg']);
				$this->settings['ihc_ap_overview_msg'] = stripslashes($this->settings['ihc_ap_overview_msg']);
		...
		private function subscription_page(){
			...
			if ($levels_str!=''){
				...
				$str .= '<thead><tr>' 
							...
							. '<td>' . __("Access", 'ihc') . '</td>'
							. '<td style="text-align:center;">' . __("Reccurence", 'ihc') . '</td>'
				...
				foreach ($levels_arr as $level_id){
					...
					if ($show_renew){			
						...
						if ($payment_type=='stripe'){
							$str .= '<span style="cursor: pointer;" onClick="ihc_stripe_renew_payment(\'' . $level_data['label'] . '\', \'' . $level_data['price'] . '\', ' . $level_id . ');">' . $renew_label . '</span> | ';
						} else {
							$str .= '<span style="cursor: pointer;" onClick="ihc_set_form_i(\'#ihc_renew_level\', \'#ihc_form_ap_subscription_page\',' . $level_id . ');">' . $renew_label . '</span> | ';
						}					
					}					
					if ($show_cancel){
				...
				$str .= '<input type="hidden" name="ihc_renew_level" value="" id="ihc_renew_level" />';//reneew
				$str .= '<input type="hidden" name="ihcaction" value="renew_cancel_delete_level_ap" />';//cancel level id
				$str .= '</form>';	
				if ($payment_type=='stripe' && $show_renew){
			...
			$coupon = (empty($fields[$num]['display_public_ap'])) ? FALSE : TRUE; 
			///PRINT COUPON FIELD
			
			$str .= ihc_user_select_level(FALSE, FALSE, $coupon);
			return $str;
		}
